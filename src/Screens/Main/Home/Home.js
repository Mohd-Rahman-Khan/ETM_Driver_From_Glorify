...
const Home = () => {
  ...
  const [showChooseRideToStartPopup, setshowChooseRideToStartPopup] = useState(false);
  ...
  //Live Tracking Code
  const stopSocketPolling = () => {
    if (socketPoll.current) {
      clearInterval(socketPoll.current); // Clear the interval
      socketPoll.current = null;
    }
  };

  const startSocketPolling = (data) => {
    if (!data || !driverCurrentCoordinate) {
      console.error("Invalid data or missing driver coordinates");
      return;  // Early return if data or coordinates are invalid
    }

    const gpsData = {
      "driverId": data?.driverData?.driverId,
      "vehicleId": data?.driverData?.vehicleId,
      "tripId": data?.tripData?.id,
      "latitude": driverCurrentCoordinate?.latitude,
      "longitude": driverCurrentCoordinate?.longitude
    }

    socketPoll.current = setInterval(() => {
      emitSocketEvent('gps-data', JSON.stringify(gpsData));
      console.log('finally<<<<<>>>>>>', JSON.stringify(gpsData));
    }, 20000);
  }

  useEffect(() => {
    const socket = getSocket();
    socket.on('driver-start-status', (response) => {
      console.log('driver-start-status', response);
      if (response?.isActive) {
        startSocketPolling(response);
      }
    })

    socket.on('gps-data-record', (error) => {
      showError(error);
      console.log('gps-data-record', error);
    })

    socket.on('driver-stop-status', (error) => {
      showError(error);
      console.log('driver-stop-status', error);
      stopSocketPolling();
    })

    socket.on('start-error-status', (error) => {
      startSocketPolling(error?.data);
      console.log('start-error-status', error);
      // showError(error?.message);
    })

    socket.on('sos-driver-started', (error) => {
      showError(error);
      console.log('sos-driver-started', error);
    })

    socket.on('sos-driver-stopped', (error) => {
      showError(error);
      console.log('sos-driver-stopped', error);
      
    })

    // Clean up the socket listeners when the component unmounts
    return () => {
      socket.off('driver-start-status');
      socket.off('gps-data-record');
      socket.off('driver-stop-status');
      socket.off('start-error-status');
      socket.off('sos-driver-started');
      socket.off('sos-driver-stopped');
    };
  })
  ...
  const srartTrip = (tripId) => {
    ...
    //this is the api to start the trip execution
    actions
      .driverSelfConsent(
        data,
          ...
          if (res?.data.length > 0) {
            ...
            if (
              ...
              navigation.navigate(navigationStrings.DRIVER_SELF_CONCENT, {
                tripId: id,
                coordinate: driverCurrentCoordinate,
              });
            } else {
              srartTrip(id);
            }
