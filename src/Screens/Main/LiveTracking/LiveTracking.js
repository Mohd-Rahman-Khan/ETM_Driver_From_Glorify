...
import { emitSocketEvent } from "../../../services/socket/socketEvents";
...
const LiveTracking = ({ route }) => {
  ...
  // Function to sort the stopList within each item
  const sortStopLists = (array) => {
    array.forEach((item) => {
      item.stopList = bubbleSort(item.stopList);
    });
    return array;
  };
  ...
  const bubbleSort = (array) => {
    ...
    do {
      swapped = false;
      for (let i = 0; i < n - 1; i++) {
        const dateA = parseDateString(array[i].expectedArivalTimeStr);
        const dateB = parseDateString(array[i + 1].expectedArivalTimeStr);
  ...
  const getDriverOngoingRide = async () => {
    actions
      ...
      .then((res) => {
        const ongoingData = sortStopLists(res?.data);
        ...
        if (res.status === 200) {
          if (ongoingData.length > 0) {
            ...
            if (ongoingData[0].tripType === "UPTRIP") {
              setData(res);

              settripId(ongoingData[0].id);
              settripType(ongoingData[0].tripType);

              settripStartTime(ongoingData[0].startTimeInMiliSec);

              let uptripLastStop =
                ongoingData[0].stopList[ongoingData[0].stopList.length - 1];
              let callingEmpList = [];
              ...
              var respArr = ongoingData[0].stopList.map((item, index) => {
              ...
              for (var i = 0; i < ongoingData[0].stopList.length; i++) {
                if (ongoingData[0].stopList[i].onBoardPassengers === null) {
                  if (ongoingData[0].stopList[i].status === "SCHEDULE") {
                    if (
                      ...
                      setshowQrCodeModalPopup(false);
                      let lastPoint =
                        ongoingData[0].stopList[
                        ongoingData[0].stopList.length - 1
                        ];
                      setlastPoint(lastPoint);

                      setshowmap(false);
                      setongoingRideData(ongoingData[0].stopList[i]);
                      ...
                      let expectedDepartureTime = moment(
                        ongoingData[0].stopList[i].expectedDepartureTime
                      );
                      let expectedArivalTime = moment(
                        ongoingData[0].stopList[i].expectedArivalTime
                      );
                      ...
                      setEmpForAttandance(
                        ongoingData[0].stopList[i].deBoardPassengers,
                        ongoingData[0].tripType
                      );
                  ...
                  if (
                    ongoingData[0].stopList[i].status === "SCHEDULE" ||
                    ongoingData[0].stopList[i].status === "ARRIVED"
                  ) {
                    setnextPickupLocation(ongoingData[0].stopList[i + 1]);
                    setongoingRideData(ongoingData[0].stopList[i]);
                    ...
                    setEmpForAttandance(
                      ongoingData[0].stopList[i].onBoardPassengers,
                      ongoingData[0].tripType
                    );
            ...
            } else if (ongoingData[0].tripType === "DOWNTRIP") {
              setData(res);
              ...
              if (getLastStopPoint.status === "DEPARTURED") {
                let getStopPointStatus = ongoingData[0].stopList.find(
                  (itemData) =>
                ...
                if (getStopPointStatus) {
                  for (let i = 0; i < ongoingData[0].stopList.length; i++) {
                    if (ongoingData[0].stopList.length === i + 1) {
                      let getEmpStatus = ongoingData[0].stopList[
                        i
                      ...
                      if (getEmpStatus === undefined || getEmpStatus === null) {
                        ...
                        setshowQrCodeModalPopup(false);
                        let lastPoint = ongoingData[0].stopList[0];
                        setlastPoint(lastPoint);
                        ...
                        if (
                          ongoingData[0].stopList[i].status === "SCHEDULE" ||
                          ongoingData[0].stopList[i].status === "ARRIVED" ||
                          ongoingData[0].stopList[i].status === "BOARDED"
                        ) {
                          let getEmpStatus = ongoingData[0].stopList[
                            i
                          ].deBoardPassengers.find(
                      ...
                      if (i === 0) {
                        ...
                        if (
                          ongoingData[0].stopList[i].deBoardPassengers.length >
                          0
                        ) {
                          if (
                            ongoingData[0].stopList[i].status === "SCHEDULE" ||
                ...
                for (let i = 0; i < ongoingData[0].stopList.length; i++) {
                  if (ongoingData[0].stopList.length === i + 1) {
                    ...
                    if (i === 0) {
                      ...
                      if (
                        ongoingData[0].stopList[i].deBoardPassengers.length > 0
                      ) {
                        if (
                          ongoingData[0].stopList[i].status === "SCHEDULE" ||
                          ongoingData[0].stopList[i].status === "ARRIVED"
  ...
  useEffect(() => {
    if (data !== "") {
      if (data?.data[0]?.status?.toUpperCase()?.trim() == 'STARTED') {
        const liveData = {
          "driverId": data?.data[0]?.driverId,
          "vehicleId": data?.data[0]?.vehicleId,
          "tripId": data?.data[0]?.id,
          "latitude": driverCurrentCoordinate?.latitude,
          "longitude": driverCurrentCoordinate?.longitude
        }
        emitSocketEvent("start-trip-driver", liveData);
        console.log('soc');
      }
    }
  }, [data]);
      ...
      actions
        ...
        .then((res) => {
          if (res.status === 200) {
            ...
            emitSocketEvent('stop-trip-driver', JSON.stringify(liveData));
            if (data?.data?.[0]?.isSSO?.toUpperCase()?.trim() === "TRUE") {
              emitSocketEvent('sos-driver-stop', JSON.stringify(liveData));
            }
            actions.newRideAssign("");
            removeUserData("coveredCord");
            setCoveredRouteCord([]);
    ...
    actions
      ...
      .then((response) => {
        ...
        if (status == 200) {
          ...
          emitSocketEvent('sos-driver', JSON.stringify(liveSosData));
  ...
  const callToEmployee = (deviceType, callType) => {
    ...
    actions
      ...
      .then((res) => {
        ...
        const sid = res?.response?.Call?.Sid;

        const IvrCallData = {
          callSid: sid,
          tripCode: empDataForCalling?.tripId,
        };

        actions
          .postIVRCallData(IvrCallData)
          .then((res) => {
            console.log("IVR Data posted successfully");
          })
          .catch((err) => {
            console.log("Error in IVR Data posting");
          });
      })
      .catch((err) => {
  ...
  const markEmpAbsent = (item) => {
    Alert.alert(
      `Are you sure you want to ${item?.passType === "ESCORT"
        ? "skip this escort"
