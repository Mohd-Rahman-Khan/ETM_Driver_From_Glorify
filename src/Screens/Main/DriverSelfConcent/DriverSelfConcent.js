...
export default function DriverSelfConcent({ route, navigation }) {
  ...
  const startSocketPolling = (res, data) => {
    const gpsData = {
      "driverId": res?.driverData?.driverId,
      "vehicleId": res?.driverData?.vehicleId,
      "tripId": res?.tripData?.tripId,
      "latitude": route.params?.coordinate?.latitude,
      "longitude": route.params?.coordinate?.longitude
    }
    socketPoll.current = setInterval(() => {
      emitSocketEvent('gps-data', gpsData);
      if (!res?.isActive) {
        stopSocketPolling();
      }
    }, 20000);
  ...
  useEffect(() => {
    ...
    socket.on('driver-start-status', (response) => {
      ...
      if (response?.data?.isActive) {
        startSocketPolling(response?.data);
      }
    })
